...
@Slf4j
@Component
@StrategyNum(superClass = PaymentStrategy.class, number = "2", describe = "默认策略:保持一个登录(不同的浏览器登录,后者剔出前者)")
public class PaymentStrategy4AlipayApp implements PaymentStrategy {
    @Override
    public Payment executePay(Payment payment) {
        Payment4Alipay payment4Alipay = (Payment4Alipay) payment;
